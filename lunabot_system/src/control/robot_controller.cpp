...
private:
  ...
  {
    if (xbox_mode_) RCLCPP_INFO(get_logger(), "XBOX MODE");
    else if (ps4_mode_) RCLCPP_INFO(get_logger(), "PS4 MODE");
    else if (switch_mode_) RCLCPP_INFO(get_logger(), "SWITCH MODE");
    else RCLCPP_ERROR(get_logger(), "NO CONTROLLER SELECTED, CHECK LAUNCH PARAMETERS");
  }
